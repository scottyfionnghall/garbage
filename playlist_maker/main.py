...
# Made it because JMusicBot can play local files but needs a .txt file 
# with all the paths to said files
#
#
...
# make_playlist opens created file and goes through a designated folder to add 
# all the songs path to said file also, it checks to add only audio files 
# by comparing file extention to exten variable which contains popular audio file formats
...
# get_filename geеs a folder name to use it as a playlist file name
...
# using that, we can assertain that if we get an error then file already exists 
# and we ask user does they want to override it if file is created we pass 'a' flag 
# to append files to the file, if file already existted we pass 'w' flag to override everything in the file
try:
